...
class TestCAMultiSelect(HelpdeskTestCase):
    ...
    def test_create_category_with_multiple_location_blocks(self):
        ...
        # Verify 4 distinct mapping records were inserted into helpdesk_ca_assignments
        new_cat = next((c for c in GLOBAL_DB_STATE.tables["helpdesk_categories"] if c["category_name"] == "Multi Block Test Category"), None)
        self.assertIsNotNone(new_cat)

        mappings = [m for m in GLOBAL_DB_STATE.tables["helpdesk_ca_assignments"] if m["category_id"] == new_cat["id"]]
    ...
    def test_create_category_duplicate_block_prevention(self):
        ...
        created_cat = next((c for c in GLOBAL_DB_STATE.tables["helpdesk_categories"] if c["category_name"] == "Existing Block Category"), None)
        ...
        mappings = [m for m in GLOBAL_DB_STATE.tables["helpdesk_ca_assignments"] if m["category_id"] == cat_id]
    ...
    def test_assign_ca_multi_block(self):
        ...
        mappings = [m for m in GLOBAL_DB_STATE.tables["helpdesk_ca_assignments"] if m["category_id"] == 1 and m["ca_id"] == 7]
        mapped_blocks = set(m["block"] for m in mappings)
